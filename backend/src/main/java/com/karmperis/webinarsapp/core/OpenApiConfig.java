...
public class OpenApiConfig {
     ...
     *
     * @return the configured OpenAPI metadata object
     */
    @Bean
    public OpenAPI customOpenAPI() {
        return new OpenAPI()
                .info(new Info()
                        .title("Webinars Application API")
                        .version("1.0.0")
                        .description("""
                                REST API for managing scheduled webinars and participant enrollments.
                                Provides endpoints for managing users, profiles, and event organization data.
                                
                                Authentication is done via JWT Bearer tokens.
                                Obtain a token from /api/v1/auth/authenticate before using secured endpoints.
                                """)
                        .contact(new Contact()
                                .name("Nikolaos Karmperis")
                                .email("karmperis@gmail.com"))
                        .license(new License()
     ...
     *
     * @return the operation customizer for global security responses
     */
    @Bean
